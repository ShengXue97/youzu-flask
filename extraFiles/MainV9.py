...
class Process:
    def __init__(self):
        ...
        self.current_ans_list = []
        self.file_attribute_list = []
        self.found_ans_options = False
    ...
    def write_data_to_document(self,document_data_list, document, qn_coord):
        ...
        for i in range(len(document_data_list)):
            ...
            # (pg_num, y_coord, self.qn_num, qn_section)
            if self.qn_num < len(qn_coord) - 1:
                next_qn_tuple = qn_coord[self.qn_num + 1]
                # print((pg_num, y_coord, next_qn_tuple[0], next_qn_tuple[1]))
                if self.pg_num > next_qn_tuple[0] or (self.pg_num == next_qn_tuple[0] and y_coord > (next_qn_tuple[1] - 5)):
                    self.qn_num = self.qn_num + 1
                    self.current_ans_list.clear()
                    self.found_ans_options = False
            ...
            ans_a = "-" if len(self.current_ans_list) <= 0 else self.current_ans_list[0]
            ans_b = "-" if len(self.current_ans_list) <= 1 else self.current_ans_list[1]
            ans_c = "-" if len(self.current_ans_list) <= 2 else self.current_ans_list[2]
            ans_d = "-" if len(self.current_ans_list) <= 3 else self.current_ans_list[3]
            ...
            if typeof == "text":
                ...
                illegal_qn_strings = ["chij", "mark", "instructions", "go on to the next page", "blank page",
                                      "question"]
                ...
                if not contains_illegal_qn_string and item != "":
                    # print((pg_num, y_coord, self.qn_num, item))
                    # ['Level', 'Question', 'isMCQ', 'A', 'B', 'C', 'D', 'Subject', 'Year', 'School', 'Exam', 'Number', 'Image', 'Image File' ]
                    if self.qn_num not in self.global_df.index:
                        # if contains_mcq_identifier or ans_a != "" or ans_b != "" or ans_c != "" or ans_d != "":
                        # global_df.loc[self.qn_num] = [paper_level, pg_num, item, "MCQ", ans_a, ans_b, ans_c, ans_d,paper_subject,
                        #                          paper_year, paper_school, paper_exam_type, self.qn_num, "No", ""]
                        # else:
                        self.global_df.loc[self.qn_num] = [paper_level, self.pg_num, item, "-", ans_a, ans_b, ans_c, ans_d,
                                                 paper_subject,
                                                 paper_year, paper_school, paper_exam_type, self.qn_num, "No", "-"]

                    else:
                        if len(self.current_ans_list) != 0 and not self.found_ans_options:
                            # If an answer option was found in this contour (So, remove the answer options from the question)
                            item = item[:first_ans_pos]
                            self.found_ans_options = True
                        elif len(self.current_ans_list) != 0 and self.found_ans_options:
                            # If an answer option was found in a contour before(Means no more text in the actual question)
                            item = ""

                        self.global_df.loc[self.qn_num, 'Question'] = self.global_df.loc[self.qn_num, 'Question'] + item
                        self.global_df.loc[self.qn_num, 'A'] = ans_a
                        self.global_df.loc[self.qn_num, 'B'] = ans_b
                        self.global_df.loc[self.qn_num, 'C'] = ans_c
                        self.global_df.loc[self.qn_num, 'D'] = ans_d

            elif typeof == "image":
                if self.qn_num not in self.global_df.index:
                    self.global_df.loc[self.qn_num] = [paper_level, self.pg_num, "-", "-", ans_a, ans_b, ans_c, ans_d, paper_subject,
                                             paper_year,
                                             paper_school, paper_exam_type, self.qn_num, "Yes", item]
                else:
                    self.global_df.loc[self.qn_num, 'A'] = ans_a
                    self.global_df.loc[self.qn_num, 'B'] = ans_b
                    self.global_df.loc[self.qn_num, 'C'] = ans_c
                    self.global_df.loc[self.qn_num, 'D'] = ans_d
                    self.global_df.loc[self.qn_num, 'Image'] = "Yes"
                    self.global_df.loc[self.qn_num, 'Image File'] = self.global_df.loc[self.qn_num, 'Image File'] + ";" + item
    ...
    def generate_document(self,imagefilename, documentdir, qn_coord):
        print("Step 2 (Output Generation): PG " + str(self.pg_num) + "/" + str(self.total_pages))
        entry = {'stage': 2, 'page' : self.pg_num, 'total' : self.total_pages, 'output' : []}
        ...
        if re.search(r'math', self.filename, re.I):
            for c in cnts:
                x, y, w, h = cv2.boundingRect(c)
            ...
            for c in cnts:
                ...
                if x / width > 0.9 or x / width < 0.1:
                    cv2.drawContours(img, [c], -1, (255, 255, 255), 20)
        thresh, cntrs, result, morph = self.get_thresh_and_contours(img, imagefilename)
        ...
        self.write_data_to_document(document_data_list, document, qn_coord)
    ...
    def section_chk(self,image, j, k):
        ...
        if len(coord_ls) == 0:
            ...
            if 0.25 < avg_loc < 0.55 and 8 < self.pg_num < 20:
                self.pg_cnt_ls.append(self.pg_num)
    ...
    def find_qn_coords(self,filenames_list):
        ...
        for filename in filenames_list:
            ...
            entry = {'stage': 1, 'page' : self.pg_num, 'total' : self.total_pages, 'output' : []}
            with open('Sessions/' + self.requestID + ".json", 'w') as outfile:
                json.dump(entry, outfile)
            ...
            for ele in target_word:
                ...
                for occ in word_occurences:
                    ...
                    x = data["left"][occ]
                    y = data["top"][occ]
                    sorted_cntr_tuples.append(("", self.pg_num, y, w, h, x))
                ...
                if area < 0.1 and area > 0.01 and y / height < 0.855 and x / width < 0.24 and w / h < 2 and w / h > 0.5:
                    ...
                    if text != "":
                        ...
                        for onetuple in sorted_cntr_tuples:
                            if abs(y - onetuple[2]) < 15 and self.pg_num == onetuple[1]:
                                probably_same_contour = True
                        if not contains_illegal_qn_string and not probably_same_contour:
                            sorted_cntr_tuples.append((c, self.pg_num, y, w, h, x / width))
            ...
            small_cntrs = []
            for c, self.pg_num, y, w, h, xw in sorted_cntr_tuples:
                if c is not "":
                    x, y, w, h = cv2.boundingRect(c)
                    new_image = img[y:y + h, x:x + w]
                else:
                ...
                small_cntrs.append((self.pg_num, y))
                self.diagram_count = self.diagram_count + 1

            for c, self.pg_num, y, w, h, xw in sorted_cntr_tuples:
                qn_coord.append((self.pg_num, y))
                # print((pg_num, y))
            ...
            ##check for section type for each page##
            self.section_chk(img, 21, 70)
            self.pg_num = self.pg_num + 1
    ...
    def main(self,pdfname, requestID):
        ...
        ## to run for the first 6 pages only (for testing)
        '''for index, page in enumerate(pages):
                self.filename = "pg_" + str(pg_cntr) + '_' + pdf_path.split('/')[-1].replace('.pdf', '.jpg')
                page.save(sub_dir + self.filename)
                pg_cntr = pg_cntr + 1
                filenames_list.append(sub_dir + self.filename)
                if index == 5:
                    break'''
        ...
        self.total_pages = len(filenames_list)
        qn_coord = self.find_qn_coords(filenames_list)
        ...
        self.pg_num = 1
        for self.filename in filenames_list:
            self.generate_document(self.filename, "OutputDocuments", qn_coord)
